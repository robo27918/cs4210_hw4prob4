...
import numpy as np
'''
Deep  Learning.  Complete  the  Python  program  (deep_learning.py) that  will learn how  to 
...
best model will be printed, together with the architecture and the learning curve
...
def build_model(n_hidden, n_neurons_hidden, n_neurons_output, learning_rate):
    ...
    #output layer
    model.add(keras.layers.Dense(n_neurons_output, activation="softmax"))   #output layer with one neural for each class 
                                                                            #and the softmax activation function since the 
...
# Moreover, the pixel intensities are represented as integers 
...
#-->add your Pyhton code here
n_neurons_output = len(class_names) # we need one neuron for each class_name
n_hidden = [2, 5, 10]
n_neurons = [10, 50, 100]
l_rate = [0.01, 0.05, 0.1]

store_best_model_param= []
store_best_model_obj = []
...
for h in n_hidden:                          #looking or the best parameters w.r.t the number of hidden layers
    for n in n_neurons:                     #looking or the best parameters w.r.t the number of neurons
        for l in l_rate:                    #looking or the best parameters w.r.t the learning rate
            ...
            class_predicted = np.argmax(model.predict(X_test), axis=-1) # predictions for the X_test samples
            ...
            curr_accurracy = correct_cnt / len(class_predicted)
            if curr_accurracy > highest_accuracy:
                highest_accuracy=curr_accurracy
                store_best_model_param.append([h,n,l])
                store_best_model_obj.append(model)
